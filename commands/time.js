...
module.exports = {
    ...
    execute(message) {
        function getTime() {
            ...
            const start = moment.utc([now.year(), now.month(), now.date(), 0, 0, 0, 0]);
            ...
            const secondsUntilImperialReset = Math.ceil(3 * 60 * 60 - elapsed % (60 * 60 * 3));
            const minutesUntilImperialReset = Math.ceil(secondsUntilImperialReset / 60);
            const hoursUntilImperialReset = Math.floor(minutesUntilImperialReset / 60);
            ...
            let secondsUntilBSAReset = Math.ceil(60 * 60 * 24) - ((elapsed - (5 * 60 * 60)) % (60 * 50 * 24));
            ...
            if (secondsIntoGameDay >= 12000) {
                ...
                const secondsUntilNightStarts = Math.ceil(12000 - secondsIntoGameDayTime);
        ...
        function createTimeEmbed(time) {
            ...
            if (time.night) {
                ...
                if (time.nighthours > 0) {
                    night = `Starts in ${time.nighthours} hours, ${time.nightminutes} minutes.`;
                }
                else {
            ...
            if (time.imperialhours > 0) {
                imperial = `Resets in ${time.imperialhours} hours, ${time.imperialminutes} minutes.`;
            }
            else {
            ...
            if (time.dailyhours > 0) {
                daily = `Reset in ${time.dailyhours} hours, ${time.dailyminutes} minutes.`;
            }
            else {
