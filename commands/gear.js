...
module.exports = {
  ...
  execute(message, args) {
    ...
    function pruneGearList() {
      ...
      GearList.destroy({
        where: { user: { [Op.notIn]: guildmembers }, guild: message.guild.id }
      });
    }
    function sendGearListReminder() {
      ...
      GearList.findAll({
        where: { user: { [Op.in]: guildmembers }, guild: message.guild.id }
      }).then(gearentries => {
        ...
        guildmembers.forEach(member => {
          message.guild.members.get(member)
            .send(`Hello! I am the Discord BDO Bot for the ${
            message.guild.name
          } Discord server. I am reminding you to register your gear in the #gear channel. You can do this by typing the following command with your values in the #gear channel:
    ...
    function createGear(discordid) {
      ...
      args.forEach(element => {
        if (element.startsWith("ap:")) {
          gear.ap = parseInt(element.substr(3));
        }
        if (element.startsWith("aap:")) {
          gear.aap = parseInt(element.substr(4));
        }
        if (element.startsWith("dp:")) {
          gear.dp = parseInt(element.substr(3));
        }
        if (element.startsWith("accuracy:")) {
          gear.accuracy = parseInt(element.substr(9));
        }
        if (element.startsWith("level:")) {
          gear.level = parseFloat(element.substr(6));
        }
        if (element.startsWith("class:")) {
          gear.class = element.substr(6).toLowerCase();
        }
        if (element.startsWith("link:")) {
          gear.link = element.substr(5);
        }
      });
      if (gear.ap < 0 || gear.ap > 999 || Number.isNaN(gear.ap)) {
        return message.reply(
          "You must enter a number between 0 and 999 for AP!\n\n\t\t\t\t\t• `!gear ap:100`\n\u200b"
        );
      }
      if (gear.aap < 0 || gear.aap > 999 || Number.isNaN(gear.aap)) {
        return message.reply(
          "You must enter a number between 0 and 999 for AAP!\n\n\t\t\t\t\t• `!gear aap:100`\n\u200b"
        );
      }
      if (gear.dp < 0 || gear.dp > 999 || Number.isNaN(gear.dp)) {
        return message.reply(
          "You must enter a number between 0 and 999 for DP!\n\n\t\t\t\t\t• `!gear dp:100`\n\u200b"
        );
      }
      if (
        gear.accuracy < 0 ||
        gear.accuracy > 999 ||
        Number.isNaN(gear.accuracy)
      ) {
        return message.reply(
          "You must enter a number between 0 and 999 for Accuracy!\n\n\t\t\t\t\t• `!gear accuracy:100`\n\u200b"
        );
      }
      if (gear.level < 0 || gear.level > 999 || Number.isNaN(gear.level)) {
        return message.reply(
          "You must enter a number between 0 and 999 for Level!\n\n\t\t\t\t\t• `!gear level:59.50`\n\u200b"
        );
      }
      ...
      if (!gear.link.startsWith("https://bdoplanner.com/")) {
        return message.reply(
          "You must enter a BDO Planner link!\n\n\t\t\t\t\t• `!gear link:https://bdoplanner.com/build`\n\u200b"
        );
      }
      gear.score = Math.round(
        (parseInt(gear.ap) + parseInt(gear.aap)) / 2 + parseInt(gear.dp)
      );
      GearList.upsert({
        user: discordid,
    ...
    function updateGear(gear, discordid) {
      ...
      args.forEach(element => {
        if (element.startsWith("ap:")) {
          newgear.ap = parseInt(element.substr(3));
        }
        if (element.startsWith("aap:")) {
          newgear.aap = parseInt(element.substr(4));
        }
        if (element.startsWith("dp:")) {
          newgear.dp = parseInt(element.substr(3));
        }
        if (element.startsWith("accuracy:")) {
          newgear.accuracy = parseInt(element.substr(9));
        }
        if (element.startsWith("level:")) {
          newgear.level = parseFloat(element.substr(6));
        }
        if (element.startsWith("class:")) {
          newgear.class = element.substr(6).toLowerCase();
        }
        if (element.startsWith("link:")) {
          newgear.link = element.substr(5);
        }
      });
      ...
      if (
        newgear.accuracy < 0 ||
        newgear.accuracy > 999 ||
        Number.isNaN(newgear.accuracy)
      ) {
        return message.reply(
          "You must enter a number between 0 and 999 for Accuracy!\n\n\t\t\t\t\t• `!gear accuracy:100`\n\u200b"
        );
      ...
      newgear.score = Math.round(
        (parseInt(newgear.ap) + parseInt(newgear.aap)) / 2 +
          parseInt(newgear.dp)
      );
      GearList.upsert({
        id: gear.id,
    ...
    if (message.channel.name !== "gear") {
      return message.reply(
        "This command can only be used in the #gear channel!"
      );
    }
    ...
    if (!args.length) {
      GearList.findOne({
        where: { user: message.author.id, guild: message.guild.id }
      }).then(gear => {
      ...
      GearList.findOne({
        where: { user: user[0].id, guild: message.guild.id }
      }).then(gear => {
        if (gear === null || gear.length === 0 || gear === undefined) {
        ...
        if (officerpermission === true) {
          if (args[1].startsWith("<@")) {
            ...
            GearList.destroy({
              ...
              if (response === 1) {
                return message.reply(
                  `Gear entry for <@${user[0].id}> has been deleted!`
                );
              } else {
                return message.reply(
                  "That user has has not registered any gear!"
                );
              }
            });
          } else {
          ...
          return message.reply(
            "This command can only be used by admins or mods!"
          );
        }
      } else {
        GearList.destroy({
        ...
        if (args.length > 1) {
          if (args[1].startsWith("<@")) {
            ...
            GearList.findOne({
              ...
              if (gear === null || gear.length === 0 || gear === undefined) {
                return message.reply(
                  "That user already does not have a gear entry!"
                );
              } else {
      ...
      if (officerpermission) {
        ...
        return message.reply(
          "Reminders have been sent to all members who have not registered their gear!"
        );
      } else {
        return message.reply(
          "This command can only be used by admins or mods!"
